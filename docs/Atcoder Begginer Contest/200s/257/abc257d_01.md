...
全ての $i$ から $j$ にジャンプするために必要なジャンプ力 $S$ を重みとする頂点 $N$ の有向グラフを作成する。

重み付き有向グラフを作成することにより、[ワーシャルフロイド法](https://ja.wikipedia.org/wiki/%E3%83%AF%E3%83%BC%E3%82%B7%E3%83%A3%E3%83%AB%E2%80%93%E3%83%95%E3%83%AD%E3%82%A4%E3%83%89%E6%B3%95)を使用して、全ての $i$ から $j$ にジャンプするために必要な最小の訓練回数を求めることができる。

ワーシャルフロイド法の計算量は $O(N^3)$ であるが、今回の制約では $2 \le N \le 200$ であるため十分間に合う。

最後に全てのジャンプ台 $n \ (1 \le n \le N)$ について、始点 $n$ から他のシャンプ台に移動するために必要な訓練回数を求め、その中から最小の値が解となる。
